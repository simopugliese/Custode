...
**Stato attuale.** Sono costruiti due pezzi. Il primo: **leggere il calendario di Google** — l'adattatore (`calendario/custode_calendario`) e il comando che chiede il permesso, senza database né job. Il secondo: **il posto dove gli eventi si conservano** — la tabella `calendar_events` (migrazione 007) e il modulo di dominio `core/custode_core/dominio/calendario.py`, che li scrive e li rilegge. I due pezzi non si parlano ancora: nessuno chiama la sincronizzazione, ed è il pezzo subito dopo. Niente tagging, niente regole.
