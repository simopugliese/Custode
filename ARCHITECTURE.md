...
*La `PATCH` è atomica.* La connessione dell'API è in autocommit (§3): senza una transazione esplicita, una richiesta con due campi di cui il secondo storto salverebbe il primo e rifiuterebbe la richiesta — il riepilogo resterebbe spostato a un giorno mai scelto mentre la risposta dice che non è cambiato niente. È lo stesso ragionamento del runner delle migrazioni, e qui costa due righe.
