...
*Il periodo del job di sincronizzazione è una **fascia** di quindici minuti.* Il worker non usa una libreria di scheduling (§5): chiede a una funzione pura cosa è dovuto adesso, e il registro `job_runs` dice se quel periodo è già stato coperto. Per un job che gira più volte al giorno il periodo non può essere una data, quindi `fascia_dovuta` restituisce l'inizio della fascia in cui cade adesso — ancorata all'ora (:00, :15, :30, :45), quindi uguale dopo ogni riavvio. Il worker si sveglia ogni cinque minuti, così la stessa fascia viene interrogata tre volte e coperta una: quei due risvegli di margine sono ciò su cui si appoggia il tentativo dopo un guasto di rete.

Un quarto d'ora perché la granularità più fine che §8.10 prevede è «dimmelo trenta minuti prima»: un impegno aggiunto adesso arriva comunque in tempo per la sua stessa regola.

*Le fasce, al contrario del settimanale e del giornaliero, non si recuperano.* Quelle funzioni guardano indietro apposta, perché un Pi spento all'ora prevista non deve perdere il riepilogo. Una fascia persa invece non contiene lavoro arretrato: contiene lo stesso lavoro di adesso. Recuperarle vorrebbe dire risincronizzare novantasei volte di fila per ottenere ciò che un giro solo ottiene subito.

*E si potano.* Il riepilogo settimanale lascia cinquantadue righe all'anno in `job_runs`; un job a fascia ne lascerebbe trentacinquemila, nella tabella più scritta del database — che è anche quella che ogni notte finisce nel backup (§9). Dato che `fascia_dovuta` guarda solo la fascia corrente, del passato del registro non si decide niente: se ne tiene un giorno, che è materiale da leggere quando qualcosa non torna.
...
- *non configurato* non è un guasto, è un modulo spento. Non si tenta e non si segna niente, così il giorno che metti le credenziali il job parte da solo invece di aspettare la fascia successiva. Che sia spento si dice **una volta all'avvio**: scoprire dopo un mese che il calendario non si è mai sincronizzato perché mancava una variabile è peggio di una riga di log in più.
- *non raggiungibile* è passeggero: la fascia **non** si segna, e si riprova al risveglio dopo — cinque minuti, non un quarto d'ora. E non ti si scrive: per un timeout di trenta secondi una notifica sarebbe solo rumore.
- *autorizzazione non valida* non si ripara da sola. La fascia **si segna**, perché ritentare tre volte in un quarto d'ora un permesso morto non serve a niente, e la fascia dopo basta ad accorgersi che nel frattempo l'hai rifatto.
