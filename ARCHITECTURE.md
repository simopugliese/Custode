...
**Mancano le auto-proposte**, e sono tutto quello che resta di questa fase: il job periodico che cerca pattern nei dati storici e ti propone una regola da approvare. Hanno bisogno di Claude e di uno storico su cui ragionare, e degli stati che la tabella ha già pronti (`proposta`, e `origine = 'ia'`), oggi non raggiungibili da nessun codice. Resta fuori anche la **correzione a parole** («no, dopo lezione ci metto un'ora, non 30 minuti»), che ha bisogno di capire *quale* regola stai correggendo e non solo cosa. Nel mezzo sta l'inferenza «sei probabilmente a casa»: il suo margine è già una manopola salvata (`check_in_minuti_dopo`), ma oggi **non lo legge nessuno** — esiste prima di chi lo userà, come `calendar_events.tipo` esisteva prima del tagging.
...
6. **Calendario + motore di contesto** (sync in sola lettura, tagging eventi, tipi di evento tuoi, impostazioni modificabili a caldo, regole dettate da te, poi le auto-proposte basate su pattern). *In corso: fatti il calendario, il tagging, i tipi che decidi tu, le impostazioni a caldo (§8) e le **regole dettate da te** (§8.10 «Stato attuale»); mancano le **auto-proposte** da pattern e la correzione di una regola a parole.*
