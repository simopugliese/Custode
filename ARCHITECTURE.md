...
*Il `.env` è il punto di partenza, non una seconda fonte di verità.* Finché non hai mai salvato, una chiave non ha riga e vale il valore dell'ambiente: è la configurazione con cui l'installazione è nata. Dal primo salvataggio vince il database e la variabile smette di contare. Non è un compromesso scelto adesso: `ImpostazioniWorker.giorno_riepilogo` lo annunciava già prima che questo modulo esistesse. Per questo la migrazione **non semina** i default — «nessuna riga» deve poter voler dire «non l'hai mai deciso tu», e seminare renderebbe indistinguibile quel caso da «deciso, e per caso coincide».
