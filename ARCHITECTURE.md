...
**Mancano le regole di contesto, e sono tutto quello che resta di questa fase.** Non esiste ancora niente: né la tabella `context_rules` di §7, né un modulo, né un job. Sono due metà distinte, e §12 le mette già in quest'ordine — prima quelle **dettate da te**, che sono logica pura e nessuna chiamata a un modello, poi le **auto-proposte** da pattern, che hanno bisogno di Claude e di uno storico su cui ragionare. Nel mezzo sta l'inferenza «sei probabilmente a casa»: il suo margine è già una manopola salvata (`check_in_minuti_dopo`), ma oggi **non lo legge nessuno** — esiste prima di chi lo userà, come `calendar_events.tipo` esisteva prima del tagging.
