...
*Chi propone il tag, e ogni quanto.* Il worker, subito dopo ogni sincronizzazione riuscita, con una **chiamata sola** per tutta la coda: il giorno che colleghi il calendario ci sono decine di serie da guardare insieme, e una chiamata per serie sarebbe lo stesso lavoro moltiplicato. La coda `gruppi_senza_tag` è anche lo **stato** del job — non ha una riga in `job_runs`, perché una coda che resta piena è già il «riprova al giro dopo». Il tagging parte dopo che la fascia del sync è stata segnata: un modello che non risponde non deve far richiamare *Google* al risveglio successivo per una lettura che era andata bene. Ed è slegato dal fatto che il sync abbia trovato qualcosa: una coda avanzata dal giro prima va svuotata anche in una giornata in cui il calendario non cambia.
