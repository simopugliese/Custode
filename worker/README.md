...
- `calendario.py` — il sync del calendario di Google, ogni cinque minuti, e il
  **tag** degli eventi nuovi: ad ogni giro chiede a DeepSeek il tipo delle
  serie che nessuno ha ancora guardato, in una chiamata sola per tutta la coda
  (§8.10). La coda è lo stato del job: non ha una riga sua in `job_runs`. I
  tipi fra cui il modello sceglie li legge da `calendar_tags` ad ogni giro e li
