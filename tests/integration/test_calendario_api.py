...
def _tagga(db_path: Path, serie: str, tipo: dom.Tipo, *, evento_id: int | None = None) -> None:
...
def test_una_proposta_dell_ia_si_distingue_da_una_tua_correzione(
    ...
    _tagga(db_path, "serie-an", dom.Tipo.ALTRO)
...
def test_le_quattro_caselle_arrivano_dal_backend(client: TestClient) -> None:
    ...
    assert corpo["tipi"] == [
        {"valore": "lezione", "label": "Lezione"},
        {"valore": "palestra", "label": "Palestra"},
        {"valore": "viaggio", "label": "Viaggio"},
...
def test_da_rivedere_mostra_una_riga_per_serie(client: TestClient, db_path: Path) -> None:
    ...
    _tagga(db_path, "serie-an", dom.Tipo.LEZIONE)
    _tagga(db_path, "", dom.Tipo.ALTRO, evento_id=3)
...
def test_da_rivedere_conta_gli_impegni_da_oggi_in_poi(client: TestClient, db_path: Path) -> None:
    ...
    _tagga(db_path, "", dom.Tipo.LEZIONE, evento_id=1)
...
def test_correggere_un_occorrenza_corregge_tutta_la_serie(
    ...
    _tagga(db_path, "serie-an", dom.Tipo.PALESTRA)
    (riga,) = client.get("/api/calendario?vista=da_rivedere").json()["daRivedere"]
